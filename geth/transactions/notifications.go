...
type SendTransactionEvent struct {
	ID        string             `json:"id"`
	Args      txutils.SendTxArgs `json:"args"`
	MessageID string             `json:"message_id"`
}

// NotifyOnEnqueue returns handler that processes incoming tx queue requests
func NotifyOnEnqueue(queuedTx *txutils.QueuedTx) {
	signal.Send(signal.Envelope{
		Type: EventTransactionQueued,
		Event: SendTransactionEvent{
			ID:        string(queuedTx.ID),
			Args:      queuedTx.Args,
			MessageID: txutils.MessageIDFromContext(queuedTx.Context),
		},
	})
}
...
func NotifyOnReturn(queuedTx *txutils.QueuedTx, err error) {
